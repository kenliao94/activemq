...
public class ActiveMQSession implements Session, QueueSession, TopicSession, StatsCapable, ActiveMQDispatcher {
    ...
    private ThreadLocal<Boolean> isInCompletionListenerCallback = new ThreadLocal<>();
    ...
    protected ActiveMQSession(ActiveMQConnection connection, SessionId sessionId, int acknowledgeMode, boolean asyncDispatch, boolean sessionAsyncDispatch) throws JMSException {
        ...
        isInCompletionListenerCallback.set(false);
    ...
    public void commit() throws JMSException {
        checkClosed();
        if (isInCompletionListenerCallback.get()) {
            throw new IllegalStateException("Session is in CompletionListener");
        }
        if (!getTransacted()) {
    ...
    public void close() throws JMSException {
        if (!closed) {
            if (isInCompletionListenerCallback.get()) {
                throw new IllegalStateException("Session is in completion listener");
            }
     ...
     * @param isProducerInCompletionListenerCallback
     * @throws JMSException
     */
    protected void send(ActiveMQMessageProducer producer, ActiveMQDestination destination, Message message, int deliveryMode, int priority, long timeToLive,
            boolean disableMessageID, boolean disableMessageTimestamp, MemoryUsage producerWindow, int sendTimeout,
            CompletionListener completionListener, ThreadLocal<Boolean> isProducerInCompletionListenerCallback) throws JMSException {
            ...
            if (completionListener==null && sendTimeout <= 0 && !msg.isResponseRequired() && !connection.isAlwaysSyncSend() && (!msg.isPersistent() || connection.isUseAsyncSend() || txid != null)) {
                ...
                if (sendTimeout > 0 && completionListener==null) {
                    ...
                    if (completionListener != null) {
                        // Make the Message object unaccessible and unmutable per Jakarta Messaging 3.1 spec section 7.3.9 and 7.3.6
                        msg.setIsMessageAccessibleAndMutable(false);
                        ...
                        wrapperCompletionListener = new CompletionListener() {
                            ...
                            public void onCompletion(Message message) {
                                try {
                                    if (message instanceof ActiveMQMessage) {
                                        ((ActiveMQMessage) message).setIsMessageAccessibleAndMutable(true);
                                    }
                                    isInCompletionListenerCallback.set(true);
                                    isProducerInCompletionListenerCallback.set(true);
                                    numIncompletedAsyncSend.doDecrement();
                                    completionListener.onCompletion(message);
                                } finally {
                                    isInCompletionListenerCallback.set(false);
                                    isProducerInCompletionListenerCallback.set(false);
                                }
                            }
