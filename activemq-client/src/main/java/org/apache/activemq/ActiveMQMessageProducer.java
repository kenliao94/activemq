...
public class ActiveMQMessageProducer extends ActiveMQMessageProducerSupport implements StatsCapable, Disposable {
    ...
    private final ThreadLocal<Boolean> isInCompletionListenerCallback = new ThreadLocal<>();
    ...
    public void close() throws JMSException {
        if (isInCompletionListenerCallback.get()) {
            throw new IllegalStateRuntimeException("The producer is in completion listener");
        }
        if (!closed) {
            dispose();
    ...
    public void send(Destination destination, Message message, int deliveryMode, int priority, long timeToLive,
        ...
        this.session.send(this, dest, message, deliveryMode, priority, timeToLive, disableMessageID,
                disableMessageTimestamp, producerWindow, sendTimeout, completionListener, isInCompletionListenerCallback);
